...
from common.generate_data import create_bankcard,create_idcard,create_name,create_userid,create_phone,create_ip
import warnings
...
class TestBankcard(unittest.TestCase):
    ...
    def test_bankcard(self,case):
        ...
        if case.data.find('register_name')>-1:
            ...
            setattr(Context,'register_name',str(userid))
        ...
        # 替换excel中的参数化bank_idcard的值（银行卡）
        if case.data.find('bank_idcard')>-1:
            bankcard = create_bankcard ()
            print('生成的银行卡号是：{}'.format(bankcard))
            case.data = case.data.replace('bank_idcard',str(bankcard))

        resp = self.ws_request.do_suds (case.url, case.data, case.method)
        log.info('得到的响应结果是：{}'.format(resp))

        # 查询数据库，将通过手机的查询到的验证码 取出来
